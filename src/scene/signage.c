...
void signageUpdate(struct Signage* signage) {
    if (signage->currentFrame >= 0 && signage->currentFrame + 1 < SIGNAGE_FRAME_COUNT) {
        ...
        // Turn the hum sound on and off with the backlight.
        if (signage->soundLoopId) {
            struct SignStateFrame frame = gSignageFrames[signage->currentFrame];
            soundPlayerAdjustVolume(signage->soundLoopId, SIGNAGE_HUM_VOLUME * (frame.backlightColor == 2));
        }
    }
}
...
void signageActivate(struct Signage* signage) {
    if (signage->currentFrame == -1) {
        ...
        signage->soundLoopId = soundPlayerPlay(soundsSignageHum, SIGNAGE_HUM_VOLUME, 1.0f, &signage->transform.position, &gZeroVec, SoundTypeAll);
    }
}
