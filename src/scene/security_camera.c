...
static struct ColliderTypeData sSecurityCameraCollider = {
    CollisionShapeTypeBox,
    &sSecurityCameraCollisionBox,
    0.0f,
    1.0f,
...
static void securityCameraLookAt(struct SecurityCamera* securityCamera, struct Vector3* target) {
    ...
    vector3Sub(target, &securityCamera->rigidBody.transform.position, &offset);

    // Don't look directly at the player
    offset.y -= 1.0f;

    float magSqrd = vector3MagSqrd(&offset);
    if (magSqrd > (SECURITY_CAMERA_RANGE * SECURITY_CAMERA_RANGE) || magSqrd < 0.01f) {
        return;
    }

    struct Quaternion invRotation;
    quatConjugate(&securityCamera->rigidBody.transform.rotation, &invRotation);
    ...
    if (fabsf(pitch.y) > 0.99f) {
        ...
        quatLerp(
            &securityCamera->armature.pose[PROPS_SECURITY_CAMERA_HEAD_BONE].rotation,
            &targetPose,
            0.1f,
            &securityCamera->armature.pose[PROPS_SECURITY_CAMERA_HEAD_BONE].rotation
        );
    ...
    quatMultiply(&sBarBoneRelative, &relativeRotation, &targetPose);
    quatLerp(
        &securityCamera->armature.pose[PROPS_SECURITY_CAMERA_BAR_BONE].rotation,
        &targetPose,
        0.1f,
        &securityCamera->armature.pose[PROPS_SECURITY_CAMERA_BAR_BONE].rotation
    );
...
void securityCameraDetach(struct SecurityCamera* securityCamera) {
    rigidBodyUnmarkKinematic(
        &securityCamera->rigidBody,
        SECURITY_CAMERA_RIGID_BODY_MASS,
        collisionBoxSolidMofI(&sSecurityCameraCollider, SECURITY_CAMERA_RIGID_BODY_MASS)
    );
